...
==================
*/
extern "C" qboolean Sys_LowPhysicalMemory() {
  //MEMORYSTATUS stat;
...
extern "C" void Sys_BeginProfiling( void ) {
}
...
extern "C" void Sys_EndStreamedFile( fileHandle_t f ) {
}

extern "C" int Sys_StreamedRead( void *buffer, int size, int count, fileHandle_t f ) {
...
================
*/
extern "C" void Sys_BeginStreamedFile( fileHandle_t f, int readAhead ) 
{
  if ( stream.file )
  {
...
sysEvent_t Sys_GetEvent( void ) {
	...
	//jens temp disabled?Sys_SendKeyEvents ();
...
void Sys_PrintBinVersion( const char* name ) {
	...
	fprintf( stdout, "\n\n%s\n", sep );
#ifdef DEDICATED
	fprintf( stdout, "BeOS Quake3 Dedicated Server [%s %s]\n", date, time );  
#else
	fprintf( stdout, "BeOS Quake3 Full Executable  [%s %s]\n", date, time );  
#endif
